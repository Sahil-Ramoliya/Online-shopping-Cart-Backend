...
// ----------------- Create checkOut -----------------
router.post("/add-deatil-of-checkout", async (req, res, next) => {
    try {
...
router.post("/paymentvarification", async (req, res) => {
    try {
        const { razorpay_order_id, razorpay_payment_id, razorpay_signature, userId, arrayofproduct, checkoutId } =
            req.body;
        if (!razorpay_order_id) {
            return res.status(404).json({
                status: 404,
                message: "Can't get razorpay_order_id."
            })
        }
        if (!razorpay_payment_id) {
            return res.status(404).json({
                status: 404,
                message: "Can't get razorpay_payment_id."
            })
        }
        if (!razorpay_signature) {
            return res.status(404).json({
                status: 404,
                message: "Can't get razorpay_signature."
            })
        }
        if (!userId) {
            return res.status(404).json({
